...
SYSTEMD_SERVICE="app-screensaver-app.service"
LEGACY_SYSTEMD_SERVICE="screensaver-app.service"
...
sed "s#Exec=screensaver-app --show#Exec=$APP_BIN_SED --daemon#" "$SCRIPT_DIR/screensaver-app.desktop" \
  > "$AUTOSTART_DIR/screensaver-app.desktop"
cat >> "$AUTOSTART_DIR/screensaver-app.desktop" <<EOF
X-GNOME-Autostart-enabled=true
X-systemd-skip=true
EOF

sed "s#ExecStart=/usr/bin/screensaver-app --daemon#ExecStart=$APP_BIN_SED --daemon#" "$SCRIPT_DIR/screensaver-app.service" \
  > "$SYSTEMD_USER_DIR/$SYSTEMD_SERVICE"

if command -v systemctl >/dev/null 2>&1; then
  systemctl --user daemon-reload || true
  systemctl --user disable --now "$LEGACY_SYSTEMD_SERVICE" || true
  rm -f "$SYSTEMD_USER_DIR/$LEGACY_SYSTEMD_SERVICE"
  systemctl --user daemon-reload || true
  systemctl --user enable --now "$SYSTEMD_SERVICE" || true
fi
