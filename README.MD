...
| `dwell dev` | Compose Hot Reload in a normal resizable always-on-top window; remembers size/position and skips daemon/tray/idle plumbing. |
...
Use `dwell dev` while iterating on UI. It should behave like a normal desktop window, remember its last size and position, not the fullscreen/transparent production overlay, and it only drops the always-on-top flag while minimized. Use `dwell show` when you need the production entrypoint from source; it must build the current checkout and should not reuse an already-running daemon window.
...
- **Profiles** — switch Work, Focus, Night, Presentation, Desk Clock, or custom profile snapshots.
...
See [`docs/profiles.md`](docs/profiles.md) for profile precedence, built-in defaults, and privacy behavior.
...
On GNOME Wayland, `dwell register` also integrates with the xdg-desktop-portal Background Apps monitor. The registered user service is named `app-dwell.service` so portals can map the background process back to `dwell.desktop`; Dwell then requests background permission and publishes a short "Idle dashboard daemon" status through the background portal.
